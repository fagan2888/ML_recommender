...
%
function [tprAtWP, auc, fpr, tpr] = evaluateMultipleMethods( labels, predictions, ...
...
for i=1:M
    ...
    [tprAtWP(i),auc,fpr,tpr] = fastROC( labels, predictions(:,i), showPlot, styles{i} );
    if showPlot
        hold on;
    end
end
...
if showPlot && ~isempty(legendNames)
    ...
    legend( legendNames, 'Location', 'south' );
end
